...
def print_histogram_and_details(
    sorted_occurrence_character_list_tuple_list, number_of_characters_for_full_histogram_bar
):
    (
        total_characters_entered,
        longest_occurrence_len,
        longest_character_list_string,
    ) = get_printing_items(sorted_occurrence_character_list_tuple_list)
    ...
    for occurrence, character_list in sorted_occurrence_character_list_tuple_list:
        character_list_string = " ".join(character_list)
        padded_character_list_string = str(character_list_string).rjust(
            longest_character_list_string, " "
        )
        ...
        histogram_bar = "*" * int(
            percentage_of_histogram_bar_to_print * number_of_characters_for_full_histogram_bar
        )
...
def main():
    ...
    if occurrence_to_character_list_dict:
        sorted_occurrence_character_list_tuple_list = sorted(
            occurrence_to_character_list_dict.items(), key=lambda x: x[0], reverse=True
        )
